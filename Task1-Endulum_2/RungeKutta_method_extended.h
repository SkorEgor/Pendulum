...
*/
class RungeKutta_method_extended
{
private:
	//------------------ПОЛЯ:-------------------//
	...
	//Доп. параметры функции
	vector <double> _param;

	//Функция движения коорднаты,
	//диф-ого ур-ния 2-ого порядка				x'' = f(t, x, x')
	double (*_move_function)(double t, double x, double x_1degree, vector<double> param);
