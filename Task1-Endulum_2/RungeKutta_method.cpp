...
//k1
double RungeKutta_method::_k1(double&  delta_t)
{
	return (_move_function(_t, _x, _x_1degree) * delta_t);
}

//k2
double RungeKutta_method::_k2(double& k1, double& delta_t)
{
...
//k3
double RungeKutta_method::_k3(double&  k1, double&  k2, double&  delta_t)
{
	return _move_function(
		_t + delta_t / 2,										//t
...
//k4
double RungeKutta_method::_k4(double&  k2, double&  k3, double&  delta_t)
{
	return _move_function(
		_t + delta_t,											//t
...
//x_(k+1)
double RungeKutta_method::_x_kPlusOne(double&  k1, double&  k2, double&  k3, double&  delta_t)
{
	return _x + _x_1degree * delta_t + (1. / 6.) * (k1 + k2 + k3) * delta_t;
}

//x_(k+1)
double RungeKutta_method::_x_kPlusOne_1degree(double&  k1, double&  k2, double& k3, double&  k4, double& delta_t)
{
...
RungeKutta_method::RungeKutta_method()
{ 
...
{
	resetAll(x0, x0_1degree, t0, move_function);
}

//-----------------Методы:------------------//

//Сброс всего
void RungeKutta_method::resetAll(
	double& x0, double& x0_1degree, double& t0,
	double (*move_function)(double t, double x, double x_1degree)
)
{
...
void RungeKutta_method::resetToBeginning(double& x0, double& x0_1degree, double& t0)
{
	//Задаем нач-ную коорд. 	x
	_x = x0;
...
state_system RungeKutta_method::getNextPosition(double& delta_t)
{
	state_system res;
